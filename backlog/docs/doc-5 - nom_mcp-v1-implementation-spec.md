...
**Widget Display setting** — single global on/off preference (rich MCP-client-rendered widgets vs plain text/JSON). Persisted in a new single-row `settings` table (`widget_display_enabled BOOLEAN`) in the same local libSQL file as domain data — deliberately **separate** from §9's startup Config, since this is runtime-mutable via a tool call while Config is read once at process start (`decision-3`). (`TASK-1.11`, `decision-3`)

**`get_goal_progress` MCP Apps widget** — the first real consumer of the setting above (`TASK-41`), establishing the pattern for later tools. Per the MCP Apps extension (SEP-1865 / modelcontextprotocol/ext-apps spec `2026-01-26`):
- When `widget_display_enabled` is `true`, `get_goal_progress`'s `Tool` declaration in `list_tools` carries `_meta: {"ui": {"resourceUri": "ui://nom-mcp/goal-progress"}}`. When `false` (the default), `meta` is `None`, identical to every other tool.
- That `ui://nom-mcp/goal-progress` resource is **not** listed in `list_resources()` — per the spec, UI-only resources are discovered via the tool's own `_meta.ui.resourceUri`, not the general resource listing. A `resources/read` call for it returns a static, self-contained `text/html;profile=mcp-app` document (no server-side templating; it renders whatever `CallToolResult` the host's `ui/notifications/tool-result` bridge delivers into the iframe).
- `call_tool("get_goal_progress")` itself is **completely unchanged** by the setting, in both states — the widget only changes how a capable host renders the existing response, never the response itself.
