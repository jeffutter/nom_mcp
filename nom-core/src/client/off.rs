...
mod tests {
    ...
    async fn test_lookup_barcode_success() {
        ...
        assert_eq!(product.product_name, Some("Widget".into()));
        assert_eq!(product.serving_size, Some(50.0));
        assert_eq!(product.nutrition_data_per, Some("per serving".into()));
        let n = product.nutriments.unwrap();
        assert_eq!(n.energy_kcal, Some(150.0));
        assert_eq!(n.energy_kcal_100g, Some(300.0));
        assert_eq!(n.proteins_100g, Some(12.0));
        assert_eq!(n.carbohydrates_100g, Some(25.0));
        assert_eq!(n.fat_100g, Some(5.0));
        assert_eq!(n.fiber_100g, Some(3.0));
    }
